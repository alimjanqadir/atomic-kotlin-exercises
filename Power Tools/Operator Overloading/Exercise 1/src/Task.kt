...
class WrapRange(val range: IntRange, private var current: Int = range.first) {
    init {
        require(current < range.last) {
            "'current' out of range: ${range.last}"
        }
    }
...
fun main() {
    ...
    capture { WrapRange(range, 6) } eq "IllegalArgumentException: " + "'current' out of range: 6"
    capture { WrapRange(range, -1) } eq "IllegalArgumentException: " + "'current' out of range: -1"
}